...
class Trade2 extends CI_Controller
{
	...
	{	
		...
		$spaymethod2   = $this->input->get('spaymethod2', TRUE);
		...
		if($spaymethod2) $condition['spaymethod2'] = $spaymethod2;
		...
		$board_cnt = $this->Trade_model2->getListCnt($condition);
		...
		$param = "page=".$page."&ssdate=".$ssdate."&sedate=".$sedate."&sselltype=".$sselltype."&stype=".$stype."&skeyword=".$skeyword."&sbrand=".$sbrand."&skind=".$skind."&sstock=".$sstock."&spaymethod=".$spaymethod."&splace=".$splace."&spaymethod2=".$spaymethod2."&saccount_conf=".$saccount_conf."&spayment_price=".$spayment_price."&smmpricecol=".$smmpricecol."&sminprice=".$sminprice."&smaxprice=".$smaxprice;
		$param2 = "&ssdate=".$ssdate."&sedate=".$sedate."&sselltype=".$sselltype."&stype=".$stype."&skeyword=".$skeyword."&sbrand=".$sbrand."&skind=".$skind."&sstock=".$sstock."&spaymethod=".$spaymethod."&splace=".$splace."&spaymethod2=".$spaymethod2."&saccount_conf=".$saccount_conf."&spayment_price=".$spayment_price."&smmpricecol=".$smmpricecol."&sminprice=".$sminprice."&smaxprice=".$smaxprice;
		
		$totsum = $this->Trade_model2->totalsum($condition);
		$board_list = $this->Trade_model2->getList($condition,$scale,$first);
		...
		foreach($temparr as $v){
		    $totSelltype[] = $this->Trade_model2->getTotSelltype($condition, $v);
		}
		...
		if($scale < $board_cnt)
		{
			...
			if($block > 1)
			{
				$paging_html .= '<li><a href="/admn/trade2?page='.($go_page-15).$param2.'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
			}
			...
			{
			$paging_html .= '<li><a href="/admn/trade2?page='.$go_page.$param2.'">'.$go_page.'</a></li>';
			}
			}
		
					if($block < $total_block) {
					$paging_html .= '<li><a href="/admn/trade2?page='.$go_page.$param2.'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
			}
			$paging_html .= '</ul>';
		}
		...
		$this->load->view('trade2/list', $data);
	}
	...
	{	
		...
		$page = $this->input->get('page', TRUE) ? $this->input->get('page', TRUE) : 1;
		$this->Trade_model2->deleteList($seq);
	...
	function write()
	{
		...
		$this->load->view('trade2/write', $data);
	}
	...
	function writeproc()
	{
      ...
      $senddate = trim($this->input->post('senddate', TRUE));
		...
		if($selltype == '' || $sellprice == ''){
			doMsgLocation('잘못된 요청입니다. 시스템 관리자에게 문의 하십시요.', "http://".$_SERVER['HTTP_HOST']."/admn/trade2");
		}
		...
		$this->Trade_model2->insertList($data);

		doMsgLocation('등록 되었습니다.', "http://".$_SERVER['HTTP_HOST']."/admn/trade2?".$param);
	}
	...
	function modify()
	{
		...
		$seq  = $this->input->get('seq', TRUE);
		$view = $this->Trade_model2->getView($seq);
		...
		$this->load->view('trade2/modify', $data);
	}
	...
	function modifyproc()
	{
		...
		$this->Trade_model2->updateList($data, $seq);

		doMsgLocation('수정 되었습니다.', "http://".$_SERVER['HTTP_HOST']."/admn/trade2?".$param);
	...
	function copyproc()
	{
		...
		if($seq){
			$this->Trade_model2->copy($seq);
			doMsgLocation('복사 되었습니다.', "http://".$_SERVER['HTTP_HOST']."/admn/trade2/?$param");
		}
	}
	...
	function excel()
	{
		...
		$board_list = $this->Trade_model2->getList($condition);
		...
		$filename = date('Y-m-d').'_위탁판매목록리스트_'.count($board_list).'건';
		...
		foreach($board_list as $k => $v){
			...
			->setCellValue("N".($k+2), $pprice)
            ->setCellValue("O".($k+2), $npay);
	
			$objPHPExcel->getActiveSheet()->getStyle("A".($k+2).":O".($k+2))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		}
	
		$objPHPExcel->getActiveSheet()->getStyle('A1:O'.($k+2))->getFont()->setSize(10);
	...
	function getPurchaseInfoFromCode(){
		...
		if($pcode){
			...
			if($purchase_seq){
				$trade_info = $this->Trade_model2->getSeqTrade($purchase_seq);
