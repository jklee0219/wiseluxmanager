...
class Overtime extends CI_Controller
{
	...
	{	
      ...
      if($sbrand) $condition['sbrand'] = $sbrand;
		...
		$board_cnt = $this->Overtime_model->getListCnt($condition);
		...
		$param2 = "&ssdate=".$ssdate."&sedate=".$sedate."&skind=".$skind."&splace=".$splace."&sstock=".$sstock."&sbrand=".$sbrand."&stype=".$stype."&skeyword=".$skeyword;
		$param = "page=".$page."&ssdate=".$ssdate."&sedate=".$sedate."&skind=".$skind."&splace=".$splace."&sstock=".$sstock."&sbrand=".$sbrand."&stype=".$stype."&skeyword=".$skeyword;
		
		$board_list = $this->Overtime_model->getList($condition,$scale,$first);
		
		$unprocessed_cnt = $board_cnt; //미처리
		$unprocessed_price = $this->Overtime_model->getUnprocessedPrice($condition)->tot_pprice; //미처리매입가격
		...
		if($scale < $board_cnt)
		{
			...
			if($block > 1)
			{
				$paging_html .= '<li><a href="/admn/overtime?page='.($go_page-15).$param2.'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
			}
			...
			{
			$paging_html .= '<li><a href="/admn/overtime?page='.$go_page.$param2.'">'.$go_page.'</a></li>';
			}
			}
		
					if($block < $total_block) {
					$paging_html .= '<li><a href="/admn/overtime?page='.$go_page.$param2.'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
			}
			$paging_html .= '</ul>';
		}
		...
		$data = array(
		   ...
		   "unprocessed_price" => $unprocessed_price,
           "goods_place" => $this->config->item('goods_place'),
		   "splace" => $splace,
		);
		
		$this->load->view('overtime/list', $data);
	}
