...
<head>
   ...
   <script src="/admn/js/attendance_modify.js?<?=time()?>"></script>
   <script>
   var qs = "<?=$param?>";
   </script>
</head>
<body>
	...
	<div id="content">
		...
		<div class="content_write">
			...
			<form name="modifyFrm" id="modifyFrm" method="post" action="/admn/attendance/modifyproc">
				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr>
							...
							<td>
								<select class="form-control input-sm" name="worker_id" required>
									...
									<option value="<?=$worker->id?>" <?=($info->worker_id == $worker->id) ? 'selected' : ''?>><?=$worker->name?> (<?=$worker->class?>)</option>
									<?php } ?>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="input-group date" style="width:200px;">
								    <input type="text" name="att_date" class="form-control input-sm" autocomplete="off" placeholder="날짜" value="<?=$info->att_date?>" required>
						...
						<tr>
							<th>비고</th>
							<td>
								<textarea name="note" class="form-control" rows="3" placeholder="비고사항을 입력하세요"><?=$info->note?></textarea>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<div class="btn_wrap">
				<button type="button" onclick="modifyFrm.submit();" class="btn btn-success">수정</button>
