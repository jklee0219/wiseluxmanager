    ...
    function html5Upload() {	
    	...
    	var imageSize = document.getElementById('image_size').value;
    	...
    	callAjaxForHTML5(tempFile,sUploadURL,imageSize, j);
    	// for(var j=0, k=0; j < nImageInfoCnt; j++) {
    	// 	tempFile = htImageInfo['img'+j];
    	// 	try{
	    // 		if(!!tempFile){
	    // 			//Ajax통신하는 부분. 파일과 업로더할 url을 전달한다.
	    // 			callAjaxForHTML5(tempFile,sUploadURL,imageSize);
	    // 			k += 1;
	    // 		}
	    // 	}catch(e){}
    	// 	tempFile = null;
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL, imageSize, uploadNum){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					...
					uploadNum++;
					tempFile = htImageInfo['img'+uploadNum];
					callAjaxForHTML5(tempFile,sUploadURL,imageSize,uploadNum);
				}
			},
			timeout : 3,
		...
		oAjax.header("file-resize",imageSize);
